...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head: int, max_ctx: int, d_model: int):
        ...
        theta: Float[Tensor, "seq d_head/2"] = (
            ...
            @ (1e4 ** (-2 * torch.arange(self.d_head // 2) / self.d_head))[None, :]
        )
    ...
    def forward(
        ...
        qkv: Float[Tensor, "batch n_seq 3_d_model"] = (
            (x @ self.qkv_proj.T) + self.qkv_bias
        ...
        q_rot: Float[Tensor, "batch n_head n_seq d_head"] = torch.stack(
            [
                q[..., 0] * self.cos_rot[:n_seq] - q[..., 1] * self.sin_rot[:n_seq],
                q[..., 0] * self.sin_rot[:n_seq] + q[..., 1] * self.cos_rot[:n_seq],
            ],
            dim=-1,
        ).reshape(batch, self.n_head, n_seq, d_head)
        k_rot: Float[Tensor, "batch n_head n_seq d_head"] = torch.stack(
            [
                k[..., 0] * self.cos_rot[:n_seq] - k[..., 1] * self.sin_rot[:n_seq],
                k[..., 0] * self.sin_rot[:n_seq] + k[..., 1] * self.cos_rot[:n_seq],
            ],
            dim=-1,
        ).reshape(batch, self.n_head, n_seq, d_head)

        # MHA
        attn_pttn = q_rot @ k_rot.transpose(-2, -1)
        attn_pttn.masked_fill_(
            torch.triu(torch.ones_like(attn_pttn), diagonal=1).bool(), -torch.inf
        )
        z: Float[Tensor, "batch n_seq d_model"] = (
            (
                nn.functional.softmax(
                    attn_pttn
                    / torch.sqrt(torch.tensor(self.d_head, dtype=torch.float)),
                    dim=-1,
                )
                @ v
            )
            .transpose(1, 2)
            .reshape(batch, n_seq, d_model)
        )
